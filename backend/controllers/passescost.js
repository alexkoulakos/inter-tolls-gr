...
exports.get = async function (req, res) {
  if(!req.params.op1_ID || !req.params.op2_ID || !req.params.date_from || !req.params.date_to){
    res.status(400).json({message: "Oops! Looks like you are missing a non-optional parameter,usage:/PassesCost/<op1_ID>/<op2_ID>/<date_from>/<date_to>?format={csv|json}"});
    return;
  }
  const schema = {
      "operators": /^[A-Z]{2}$/,
  ...
  try{
    ...
    var ans = {'op1_ID' : op1, 'op2_ID' : op2, 'RequestTimestamp' : ts, 'PeriodFrom' : periodFrom, 'PeriodTo' : periodTo, 'NumberOfPasses' : NumberOfPasses, 'PassesCost' : Math.round(PassesCost*100)/100};
