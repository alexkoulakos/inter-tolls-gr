...
exports.post = function (req, res) {
    ...
    mongodb.MongoClient.connect(uri, { useUnifiedTopology: true },function(error,client) {
        ...
        if (error) throw error;
            if (client) {
                // console.log('Inside resetstations: DB Connected!');
                db = client.db();
                db.collection('stations').drop(function (err, result) {
                    // if (result) 
                        // console.log("Collection Stations successfully deleted.");
                });
                const stationsFile = __dirname + "/files/stations.csv";
